...
import { FileText, Download, Trash2, Brain, Loader2, Settings as SettingsIcon } from 'lucide-react';
...
export default function Reports() {
    ...
    const fetchProfiles = async () => {
        try {
            ...
            if (res.data.length > 0) {
                // creating a default selection if none exists
                if (!selectedServerId) setSelectedServerId(res.data[0].id);
            }
        } catch (error) {
    ...
    return (
        <div className="space-y-6 h-[calc(100vh-140px)] flex flex-col">
            ...
            <div className="flex-1 flex gap-6 overflow-hidden flex-col md:flex-row min-h-0">
                {/* List of Reports */}
                <div className="w-full md:w-1/3 overflow-y-auto pr-2 space-y-4 shrink-0">
                                        ...
                                        <ul className="space-y-1">
                                            {selectedReport.aggregated_data.storage_changes.map((change: any, i: number) => (
                                                <li key={i} className="text-sm text-muted-foreground">
                                                    <span className="font-mono">{change.mountpoint}</span>: {formatBytes(change.from_used_gb * 1024 * 1024 * 1024)} &rarr; {formatBytes(change.to_used_gb * 1024 * 1024 * 1024)} ({change.change_gb > 0 ? '+' : ''}{change.change_gb} GB)
                                                </li>
                                            ))}
                                        </ul>
                                    </div>

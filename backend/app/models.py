...
class Tenant(Base):
    ...
    id = Column(String(50), primary_key=True)  # tenant identifier (e.g., 'default', 'acme-corp')
    name = Column(String(255), nullable=False)  # display name
    plan = Column(String(50), default='free')  # free, pro, enterprise
    status = Column(String(20), default='active')  # active, suspended, deleted
    resource_quota = Column(JSON().with_variant(JSONB, "postgresql"), default={})  # resource limits
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())
    tenant_metadata = Column(JSON().with_variant(JSONB, "postgresql"), default={})  # flexible metadata
    
    # Relationships
    user_memberships = relationship("UserTenantMembership", back_populates="tenant", cascade="all, delete-orphan")


class UserTenantMembership(Base):
    """Junction table for user-tenant many-to-many relationship with roles"""
...
class Plugin(Base):
    ...
    plugin_tenant_metadata = Column(JSON().with_variant(JSONB, "postgresql"), default={})  # Full plugin metadata
...
class ApplicationBlueprint(Base):
    ...
    blueprint_tenant_metadata = Column(JSON().with_variant(JSONB, "postgresql"), default={})  # tags, version, author, documentation_url, icon_url
